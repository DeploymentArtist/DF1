...
Public Const Version = "5.0.1641.0"
...
class Logging
	...
	Public Function CopyLog()
		...
		If oFSO.FileExists(oEnvironment.Item("SMSTSLogPath_Cache") & "\SMSTS.LOG") then
			oLogging.CreateEntry "Copying " & oEnvironment.Item("SMSTSLogPath_Cache") & "\SMSTS.LOG to " & sNewLogFolderName & "\SMSTS.LOG", LogTypeInfo
			oFSO.CopyFile oEnvironment.Item("SMSTSLogPath_Cache") & "\SMSTS.LOG", sNewLogFolderName & "\", True
		...
		If oFSO.FileExists(oENV("TEMP") & "\smsts.log") Then

			oUtility.VerifyPathExists sNewLogFolderName & "\"
			OLogging.CreateEntry "Copying " & oENV("TEMP") & "\smsts.log to " & sNewLogFolderName & "\", LogTypeInfo
			oFSO.CopyFile oENV("TEMP") & "\smsts.log",  sNewLogFolderName & "\", True
		End If
...
Class Environment
	...
	Function SetDAT(sVariable, sNew)
		...
		If osdv4 is Nothing OR oEnvironment.Item("_SMSTSPACKAGENAME") = "Lite Touch Installation" Then
			If oFSO.FolderExists(PersistPath) then
				...
				' Load the XML file containing the variable values
				...
				' If this is a secure value, obfuscate the text using Base64 encoding
				...
				Select Case Ucase(sVariable)
					Case "USERID", "USERPASSWORD", "USERDOMAIN", "DOMAINADMIN", "DOMAINADMINPASSWORD", "DOMAINADMINDOMAIN", _
						...
						sNew = oStrings.Base64Encode(sNew)
				End Select					
				...
				Set oCDATA = oVariables.createCDATASection(sNew)
	...
	Public Property Get Item(sVariable)
		...
		Dim tmpItem
		tmpItem = GetOSDV4(sVariable)
		Item = GetOSDV4(sVariable)
		If GetOSDV4("DeploymentMethod") = "SCCM" Then
			Exit Property
		End If


		'First check to see if the item is encoded, if so decode it

		If Item = "" then
			tmpItem = GetDat(sVariable)
			...
			If Item <> "" then
				...
				Select Case Ucase(sVariable)
					Case "USERID", "USERPASSWORD", "USERDOMAIN", "DOMAINADMIN", "DOMAINADMINPASSWORD", "DOMAINADMINDOMAIN", _
						 "ADMINPASSWORD", "BDEPIN", "TPMOWNERPASSWORD", "ADDSUSERNAME", "ADDSPASSWORD", _
						 "SAFEMODEADMINPASSWORD", "USERNAME", "USERPASSWORD", "PRODUCTKEY"
						Item = oStrings.Base64Decode(Item)
				End Select
	...
	Public Property Let Item(sVariable, sNew)
		...
		oEnv(sVariable) = sNew
		If SetOSDV4(sVariable,SNew) or SetDat(sVariable, SNew) Then
...
Class Utility
	...
	Function MapNetworkDriveEx (sShare, sDomID, sDomPwd, iLogType )
		...
		For sDrive = asc("Z") to asc("C") step -1
			...
			' Updated code for Multiple connections to a server or shared resource bug
			' Edited 2009-09-24 by Johan Arwidmark
...
Class FileHandling
	...
	Function RemoveFolderEx(sPath, bLogging )
		...
		For each oFile in oFSO.GetFolder(sPath).Files
			DeleteFileEx oFile.name, bLogging
		Next
